...
async function loadEvents() {
  try {
    const res = await fetch('api.php?v=' + Date.now());
...
const FALLBACK_EVENTS = [
  { id:1, title:"GBV Awareness Training Workshop", description:"A 20-day workshop providing GBV awareness training to community leaders and health workers across Sool region.", date:"2024-03-15", location:"Lasanod, Sool", type:"event", category:"Women Empowerment", photo:"", report_url:"" },
  { id:2, title:"Teacher Training Program – Sanaag Schools", description:"Targeted training for teachers in conflict-affected schools to enhance instruction quality and support displaced students.", date:"2024-04-08", location:"Erigavo, Sanaag", type:"program", category:"Education", photo:"", report_url:"" },
  { id:3, title:"GBV Research Publication Released", description:"Saharla Foundation releases findings from its comprehensive GBV Prevalence, Vulnerability, and Psychological Trauma Assessment.", date:"2024-04-20", location:"Mogadishu, Somalia", type:"news", category:"Research", photo:"", report_url:"" },
  { id:4, title:"Youth Vocational Training Graduation", description:"Graduation ceremony for 60 youth participants who completed vocational training in carpentry, tailoring, and mobile phone repair.", date:"2024-05-12", location:"Cayn Region", type:"event", category:"Youth Development", photo:"", report_url:"" },
  { id:5, title:"Community Tree-Planting Drive", description:"Community-wide reforestation initiative with over 500 trees planted by youth volunteers and school children.", date:"2024-06-03", location:"Sool & Sanaag", type:"program", category:"Climate Resilience", photo:"", report_url:"" },
  { id:6, title:"Women Empowerment Research Phase 2", description:"Saharla Foundation launches Phase 2 study: Empowering Women and Girls in Conflict-Affected Areas across SSC regions.", date:"2024-06-22", location:"SSC Regions", type:"news", category:"Research", photo:"", report_url:"" }
];
...
function renderFeaturedEvents(events) {
  ...
  if (!strip) return;
  const latest = [...events].sort((a,b) => new Date(b.date) - new Date(a.date)).slice(0, 3);
  strip.innerHTML = '';
  latest.forEach((ev, i) => {
    ...
    const imgHtml = ev.photo
      ? `<div class="fe-img" style="background-image:url('${esc(ev.photo)}')"></div>`
      : `<div class="fe-img fe-img-empty"><i class="fas fa-image"></i></div>`;
    const reportBtn = ev.report_url
      ? `<a href="${esc(ev.report_url)}" target="_blank" rel="noopener" class="fe-report"><i class="fas fa-file-pdf"></i> View Report</a>`
      : '';
    card.innerHTML = `
      ${imgHtml}
      <div class="fe-body">
        <div class="fe-top">
          <span class="fe-badge ${esc(ev.type)}">${esc(cap(ev.type))}</span>
          <span class="fe-date"><i class="fas fa-calendar"></i> ${d.day} ${d.mo}</span>
        </div>
        <h4>${esc(ev.title)}</h4>
        <p>${esc((ev.description||'').substring(0,100))}…</p>
        <div class="fe-foot">
          <span class="fe-loc"><i class="fas fa-map-marker-alt"></i>${esc(ev.location||'')}</span>
          ${reportBtn}
        </div>
      </div>`;
    card.style.cssText = 'opacity:0;transform:translateY(24px);transition:opacity .55s ease,transform .55s ease';
    strip.appendChild(card);
    setTimeout(() => { card.style.opacity='1'; card.style.transform='none'; }, 100 + i*120);
  });
}
...
function renderEvents(filter = 'all') {
  ...
  sorted.forEach((ev, i) => {
    ...
    const imgHtml = ev.photo
      ? `<div class="ec-photo" style="background-image:url('${esc(ev.photo)}')"></div>`
      : '';
    const reportBtn = ev.report_url
      ? `<a href="${esc(ev.report_url)}" target="_blank" rel="noopener" class="ec-report"><i class="fas fa-file-pdf"></i> View Report</a>`
      : '';
    card.innerHTML = `
      ${imgHtml}
      <div class="ec-head">
